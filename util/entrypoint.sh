...
if [ -n "$IFACE" ]; then
    ...
    if [ ! -r "$dhcpd6_conf" ]; then
        echo "Please ensure '$dhcpd6_conf' exists and is readable."
        echo "Run the container with arguments 'man dhcpd6.conf' if you need help with creating the configuration."
        exit 1
    fi
    ...
    #run ipv6
    $run /usr/sbin/dhcpd -6 -f -d --no-pid -cf "$data_dir/dhcpd6.conf" -lf "$data_dir/dhcpd6.leases" $IFACE
else
    # Run another binary
    $run "$@"
fi
